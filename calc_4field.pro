...
; needed for eazy
  ...
  if not keyword_set(SAM) then td = selStandard( cdfs) else $
     td = where( cdfs.ra le 1./sqrt(2) and cdfs.dec le 1./sqrt(2) and $
  ...
  for i=0,n_elements(td)-1 do begin
     ...
     if useZ[td[i]] ge minZred then begin
        ...
        if n_elements(zd) ge max(nth) then begin
           calc_nn, cdfs[zd].ra, cdfs[zd].dec, nth=nth, Dn=tDn, $
                    rarcs=rkpc/kpc2arcs, nR=tnR

     ;; find the galaxy:
           xx = (where( cdfs[zd].ra eq cdfs[td[i]].ra and cdfs[zd].dec eq cdfs[td[i]].dec))[0]
           nR[*,td[i]] = tnR[*,xx]
           Dn[*,td[i]] = tDn[*,xx]
        endif
     endif 
